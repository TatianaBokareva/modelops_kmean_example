...
def score(context: ModelContext, **kwargs):
    ...
    #predictions_pdf = KMeansPredict_out.result.to_pandas(all_rows=True).rename(columns={"prediction": target_name}).astype({target_name: int})
    ...
    # Prepare the predictions DataFrame for database insertion
     #  predictions_pdf["job_id"] = context.job_id  # Add job_id to track the execution
      # predictions_pdf[entity_key] = features_pdf[entity_key]  # Set entity key from the features_pdf
